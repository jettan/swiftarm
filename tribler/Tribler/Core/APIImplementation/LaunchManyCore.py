...
class TriblerLaunchMany(Thread):
    ...
    def start_dispersy(self, config, keypair):
        def load_communities():
            if getattr(sys, 'argv', False) and sys.argv[0].endswith("dispersy-channel-booster.py"):
                schedule = []
        ...
        # define auto loads
        self.dispersy.define_auto_load(AllChannelCommunity, (self.session.dispersy_member,), {"auto_join_channel":True} if getattr(sys, 'argv', False) and sys.argv[0].endswith("dispersy-channel-booster.py") else {})
